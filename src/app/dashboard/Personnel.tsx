...
const StaffKpiCard: React.FC<StaffKpiCardProps> = ({ icon, title, value, detail, colorClass }) => (
...
const StaffListTable: React.FC<StaffListTableProps> = ({ members, onSelectMember }) => (
...
const StaffDetailSheet: React.FC<StaffDetailSheetProps> = ({ member, onClose }) => {
      ...
      >
        ...
        <div className="flex-1 p-6 space-y-6 overflow-y-auto">
          ...
          <div className="bg-white p-5 rounded-xl border">
            ...
            <Bar data={performanceData} options={{ responsive: true, plugins: { legend: { position: 'top' as const } } }} />
          </div>
            ...
            <div className="space-y-3">
              {member.assignedTasks.map(task => (
                <div key={task.id} className="p-3 rounded-lg bg-gray-50/80 flex justify-between items-center">
                  ...
                  <span className={`text-xs font-semibold px-2 py-1 rounded-full ${task.status === 'Terminé' ? 'bg-green-100 text-green-700' :
...
const AddStaffModal: React.FC<AddStaffModalProps> = ({ isOpen, onClose, onAddStaff }) => {
  const [formData, setFormData] = useState({
    name: '', 
    role: 'Manutentionnaire' as StaffRole, 
    workZone: 'Zone B - Stockage' as 'Zone A - Réception' | 'Zone B - Stockage' | 'Zone C - Expédition', 
    phone: '', 
    email: ''
  });
  ...
  const handleChange = (e: ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    setFormData(prev => ({ 
      ...prev, 
      [name]: name === 'role' ? value as StaffRole : 
               name === 'workZone' ? value as typeof formData.workZone : 
               value 
    }));
  };
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="p-6 space-y-4">
            ...
            >
              ...
              <option value="Accueil Client">Accueil Client</option>
              <option value="Préparateur Commandes">Préparateur Commandes</option>
            </select>
            <select 
              name="workZone" 
              ...
              className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500"
            >
              <option value="Zone A - Réception">Zone A - Réception</option>
              <option value="Zone B - Stockage">Zone B - Stockage</option>
              <option value="Zone C - Expédition">Zone C - Expédition</option>
            </select>
            <input 
              type="tel" 
              name="phone" 
    ...
    <div className="p-8 bg-gray-50 flex-1 space-y-8">
      ...
      <div className="flex justify-between items-center">
        ...
        <button 
          ...
          className="flex items-center space-x-2 bg-green-600 hover:bg-green-700 text-white font-semibold py-3 px-5 rounded-xl transition-all shadow-md"
        >
      ...
      <div className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 flex flex-col md:flex-row justify-between items-center gap-4">
        ...
        <div className="relative w-full md:w-auto">
          ...
          <select 
            ...
            className="pl-12 pr-8 py-3 w-full md:w-56 border border-gray-200 rounded-xl appearance-none bg-gray-50 focus:ring-2 focus:ring-green-500"
          >
